...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is BoolType:
            ...
            return "[" + "["*(len(inType.dimens)-1) + self.getJVMType(inType.eleType if not isinstance(inType.eleType, StructLiteral) else inType.eleType.name)
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        elif type(typ) is NilLiteral:
            frame.push()
            return self.jvm.emitPUSHNULL()  # Sinh lệnh 'aconst_null' cho null reference
        else:
    ...
    def emitALOAD(self, in_, frame):
        ...
        if type(in_) is IntType:
            ...
            type(in_) is cgen.ArrayType or \
            type(in_) is cgen.ClassType or \
            type(in_) is StringType or \
            type(in_) is InterfaceType or \
            type(in_) is StructType or \
            type(in_) is Id:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        if type(inType) is IntType or type(inType) is BoolType:
            ...
            type(inType) is cgen.ArrayType or \
            type(inType) is cgen.ClassType or \
            type(inType) is StringType or \
            type(inType) is cgen.StructType or \
            type(inType) is cgen.InterfaceType:
    ...
    * @param in the type descriptor of the method.
    ...
    def emitINVOKEINTERFACE(self, lexeme, in_, frame):
        # lexeme: Tên đầy đủ của method, ví dụ: "Course/study"
        # in_: MType chứa list param type và return type
        # frame: Frame

        typ = in_
        
        # Pop tất cả đối số khỏi operand stack
        for _ in typ.partype:
            frame.pop()

        # Pop thêm biến this (interface object)
        frame.pop()

        # Nếu return type không phải void, push 1 giá trị lên operand stack
        if not isinstance(typ.rettype, VoidType):
            frame.push()

        # Số lượng đối số bao gồm cả 'this'
        num_args = len(typ.partype) + 1

        return self.jvm.emitINVOKEINTERFACE(lexeme, self.getJVMType(in_), num_args)
    ...
    *   @param in the type of the operands.
    ...
    def emitSTRINGCONCATINATION(self, frame, leftCode, rightCode):
        return ''.join([
            self.jvm.emitNEW("java/lang/StringBuilder"),
            self.jvm.emitDUP(),
            self.jvm.emitINVOKESPECIAL("java/lang/StringBuilder/<init>", "()V"),
            leftCode,
            self.jvm.emitINVOKEVIRTUAL("java/lang/StringBuilder/append", "(Ljava/lang/String;)Ljava/lang/StringBuilder;"),
            rightCode, 
            self.jvm.emitINVOKEVIRTUAL("java/lang/StringBuilder/append", "(Ljava/lang/String;)Ljava/lang/StringBuilder;"),
            self.jvm.emitINVOKEVIRTUAL("java/lang/StringBuilder/toString", "()Ljava/lang/String;")
        ])
    ...
    *   generate ior
    ...
    def emitSTRINGCOMPARATION(self, op, frame):
        ...
        # Xử lý kết quả với từng op
        if op == "==":
            result.append(self.jvm.emitIFEQ(labelTrue))
        elif op == "!=":
            result.append(self.jvm.emitIFGT(labelTrue) or self.jvm.emitIFLT(labelTrue))
        elif op == "<":
    ...
    def emitNEWARRAY(self, in_, frame):
        ...
        if isinstance(in_, IntType):
            ...
            return self.jvm.emitNEWARRAY(in_.name)
        else:
            raise IllegalOperandException(f"Unsupported type in emitNEWARRAY: {in_}")

    """ 
    Xử lý mảng chứa reference types như: int[], String, Object...
    Nếu là mảng nhiều chiều, cần truyền type chuẩn hóa: "[I", "[Ljava/lang/String;", etc. 
    """
    def emitANEWARRAY(self, in_, frame):
        ...
        elif isinstance(in_, cgen.ClassType):
            return self.jvm.emitANEWARRAY(in_.classname)
        elif isinstance(in_, Id):
            return self.jvm.emitANEWARRAY(in_.name)
        else:
            # fallback: giả định là reference type đơn giản
            jvm_type = self.jvm.getJVMType(in_)
            return self.jvm.emitANEWARRAY(jvm_type)
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        if type(in_) is IntType or type(in_) is BoolType:
            ...
            type(in_) is StringType or \
            type(in_) is cgen.ClassType or \
            type(in_) is cgen.InterfaceType or \
            type(in_) is cgen.StructType:
            frame.pop()
    ...
    *   .super java/lang/Object<p>
    ...
    def emitPROLOG_INTERFACE(self, name, parent):
        #name: String
        #parent: String
    ...
    *   @param in the code to be printed out
    ...
    def emitMULTIANEWARRAY(self, int_, frame):
        # for _ in range(int_):
        #     frame.pop()
        frame.push()
        return JasminCode.INDENT + "multianewarray" + str(int_) + JasminCode.END

    def emitINVOKESPECIAL_CONSTRUCTOR(self, classname: str, param_types: List[Type], frame):
        typedes = "(" + "".join([self.getJVMType(pt) for pt in param_types]) + ")V"
        return f"\tinvokespecial {classname}/<init>{typedes}\n"
